...
public class ShaderOptionVariableConstHook {
	...
	public static ShaderOption parseOption(String line, String path) {
		...
		reader.readWhile(FastStringReader::isWhitespace);
		String type = reader.readMatching(FastStringReader::isWord);
		if (!type.equals("float") && !type.equals("int")) return null;
		reader.readWhile(FastStringReader::isWhitespace);
		...
		return new ShaderOptionVariableConst(name, type, description, value, values, null);
	}

	public static boolean matchesLine(ShaderOptionVariableConst option, String line) {
		FastStringReader reader = new FastStringReader(line);
		reader.readWhile(FastStringReader::isWhitespace);
		if (!reader.read("const")) return false;
		reader.readWhile(FastStringReader::isWhitespace);
		String type = reader.readMatching(FastStringReader::isWord);
		if (!type.equals("float") && !type.equals("int")) return false;
		reader.readWhile(FastStringReader::isWhitespace);
